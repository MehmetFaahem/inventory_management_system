...
<script type="text/javascript">
    ...
    var orderdataTable = $('#order_data').DataTable({
        ...
        "columnDefs":[
            {
                "targets":[4, 5, 6, 7, 8],
                "orderable":<?php echo ($_SESSION["type"] == 'master') ? 'true' : 'false'; ?>
            }
        ],
        "pageLength": true
    });
    ...
    function addProductRow(count = '') {
        ...
        html += '<button type="button" class="btn btn-' + (count == '' ? 'success' : 'danger') + ' btn-xs ' + (count == '' ? 'add_more' : 'remove') + '">' + (count == '' ? '+' : '-') + '</button>';
        html += '</div>';
    ...
    $(document).on('click', '.update', function(){
        var inventoryOrderId = $(this).attr("id");
        var btnAction = 'fetch_single';
        $.ajax({
            url:"order_action.php",
            method:"POST",
            data:{inventory_order_id:inventoryOrderId, btn_action:btnAction},
            dataType:"json",
            success:function(data)
            {
                $('#orderModal').modal('show');
                ...
                $('#inventory_order_id').val(inventoryOrderId);
                $('#action').val('Edit');
                $('#btn_action').val('Edit');
            }
        });
    ...
    $(document).on('click', '.delete', function(){
        ...
        if(confirm("Are you sure you want to change status?"))
        {
            $.ajax({
                url:"order_action.php",
                method:"POST",
                data:{inventory_order_id:inventoryOrderId, status:status, btn_action:btnAction},
                success:function(data)
                {
